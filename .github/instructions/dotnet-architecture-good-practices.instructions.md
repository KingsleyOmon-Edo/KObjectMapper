---
description: "DDD process discipline and verification checklist — subordinate to .github/copilot-instructions.md for all technical/package-specific rules"
applyTo: '**/*.cs,**/*.csproj,**/Program.cs,**/*.razor'
---

# DDD Systems & .NET Guidelines — Process Layer

> **Scope note:** This file governs *process discipline* (thinking-out-loud, verification checklists, general DDD/SOLID reasoning). It does **not** override any concrete technical rule in `.github/copilot-instructions.md` — the Techbiquity package APIs, error-handling model (`Result<T>`/`Error` at the Application boundary, exceptions in the Domain layer), Specification-pattern repositories, MediatR-based CQRS, and the `MethodUnderTarget_Condition_ExpectedBehavior` test naming convention are all defined there and take precedence if any wording here appears to conflict.
...
2.  **Review Against Guidelines** - Explicitly check:
    ...
    * Will tests follow the `MethodUnderTarget_Condition_ExpectedBehavior()` naming pattern (per `.github/copilot-instructions.md`)?
...
* **Test Naming Convention**: Use the `MethodUnderTarget_Condition_ExpectedBehavior()` pattern defined in `.github/copilot-instructions.md`.
...
* Test cases using the `MethodUnderTarget_Condition_ExpectedBehavior()` pattern (see `.github/copilot-instructions.md`).
...
```csharp
// AAA structure via blank-line separation only — never emit explicit
// // Arrange, // Act, // Assert comment markers (see .github/copilot-instructions.md)
[Fact(DisplayName = "Descriptive test scenario")]
public void MethodUnderTarget_Condition_ExpectedBehavior()
{
...
1.  **Verify naming follows pattern**: `MethodUnderTarget_Condition_ExpectedBehavior()` (defined in `.github/copilot-instructions.md`)
...
* **Validation Strategy**: "I have used the correct technique for the layer — FluentValidation for request shape, `Guard`/exceptions for construction invariants, `Result`/`Error` for predictable business outcomes — per the Validation, Domain Guards, and Error Handling section of `.github/copilot-instructions.md`."
* **Event Handling**: "I have confirmed domain events are properly published and handled."

### Implementation Quality Validation

* **Test Coverage**: "I have written comprehensive tests following `MethodUnderTarget_Condition_ExpectedBehavior()` naming (per `.github/copilot-instructions.md`)."
...
* Follow the `MethodUnderTarget_Condition_ExpectedBehavior()` test naming pattern (per `.github/copilot-instructions.md`).
